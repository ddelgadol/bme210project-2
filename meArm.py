...
class meArm():
    value = 0
    def __init__(self,
            sweepMinBase     = 123, sweepMaxBase     = 533, angleMinBase     =  1.22, angleMaxBase     = -1.57,
            sweepMinShoulder = 113, sweepMaxShoulder = 318, angleMinShoulder =  1.92,  angleMaxShoulder =  0.35,
            sweepMinElbow    = 553, sweepMaxElbow    = 369, angleMinElbow    =  0.35, angleMaxElbow    = -1.05,
            sweepMinGripper  = 574, sweepMaxGripper  = 225, angleMinGripper  =  -0.79, angleMaxGripper  =  0.70):
    ...
    def openGripper(self):
        ...
        self.pwm.setPWM(self.gripper, 0, self.angle2pwm("gripper", pi/2.0))
        time.sleep(0.1)
    def closeGripper(self):
        """Close the gripper, grabbing onto anything that might be there"""
        self.pwm.setPWM(self.gripper, 0, self.angle2pwm("gripper", -pi/12.5))
        time.sleep(0.1)
